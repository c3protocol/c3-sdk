import { Deployer, FieldType } from "./Deployer";
...
describe('Compile Stateless Contract', function() {
    ...
    it('Simple dynamic statess contract', function() {
        ...
        const [parsedCode, parameters] = Deployer.parseCode(sourceCode, subMap)

        expect(parsedCode).toBe(expectedParsedCode)
        expect(parameters).toHaveLength(3)
        expect(parameters[0].name).toBe('TEST')
        expect(parameters[0].type).toBe(FieldType.STRING)
        expect(parameters[1].name).toBe('ADR')
        expect(parameters[1].type).toBe(FieldType.ADDRESS)
        expect(parameters[2].name).toBe('RETURN')
        expect(parameters[2].type).toBe(FieldType.UINT)
    })
